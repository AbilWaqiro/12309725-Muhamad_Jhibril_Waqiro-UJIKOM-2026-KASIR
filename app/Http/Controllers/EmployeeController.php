...
class EmployeeController extends Controller
{
    ...
    public function storeTransaction(Request $request)
    {
        ...
        $isMember = $request->input('customer_type') === 'member';
        $usePoints = $isMember && $request->has('use_points') && $request->boolean('use_points');

        $discount = $usePoints ? ($totalPrice * 0.01) : 0;
        ...
        if ($totalPay < $finalPrice) {
            ...
            return back()->withErrors([
                'total_pay' => $usePoints
                    ? 'Payment is less than the total after discount.'
                    : 'Payment must be at least the total price.'
            ])->withInput();
        }
